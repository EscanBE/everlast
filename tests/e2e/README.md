...
# previous node state folder. By default this is './build/.everlast:/root/.everlast'
...
# By default this is 'everlast_80808-1'
CHAIN_ID
```
...
the test suite exports `/.everlast` folder from the docker container
...
will be pulled from [DockerHub](https://hub.docker.com/r/EscanBE/everlast/tags).
If it is not specified,
the test suite retrieves the second-to-last upgrade version
from the local codebase (in the `everlast/app/upgrades` folder)
...
```log
ok  	github.com/EscanBE/everlast/v12/tests/e2e	174.137s.
```
...
Error:  Received unexpected error:
        ...
        main.appCreator.newApp({{{0x3399b40, 0xc000ec1db8}, {0x33ac0f8, 0xc00...
            github.com/EscanBE/everlast/v12/cmd/evld/root.go:243 +0x2ca
...
```log
CONTAINER ID   IMAGE
9307f5485323   everlast:local    <-- upgraded node
f41c97d6ca21   everlast:v12.0.0   <-- initial node
```
